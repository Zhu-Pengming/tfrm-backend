...
**TFRM 是一个面向旅游 B2B 的资源协同与报价 SaaS，支持 AI 入库、私有/公共双库和多机构合作。**

TFRM（Travel Fragment Resource Management）通过 AI 技术和双库协同机制，解决旅游行业资源分散、报价低效、协作割裂的核心痛点。

**核心价值**：
- 🤖 **AI 智能入库**：自动从文本/图片/PDF/DOCX 中提取结构化资源信息，支持 7 大品类
- 🏢 **双库协同**：私有库 + 公共库，机构既是资源提供方也是采购方
- 🤝 **合作网络**：申请-审核-合作机制，实现资源安全共享与复用
- 💰 **灵活定价**：成本价/销售价分离，支持合作方协商价与加价因子
- 📋 **快照报价**：固化价格与属性，历史报价不受后续变更影响
- 🔔 **变更通知**：合作 SKU 变更自动站内信通知
- �️ **统一标签**：兴趣标签 + 服务标签，支持多维度筛选

**适用场景**：
- 地接社发布资源到公共库，供组团社申请合作
- 批发商管理私有库，组合资源生成客户报价
- 定制社浏览公共库，快速引用合作方优质资源
...
- 📝 支持文本、图片、PDF、DOCX 多种格式输入
- 🤖 **目前默认使用 `kimi-k2.5` 作为多模态模型；文件在后端先抽取文本，再调用 Chat API，不依赖 Kimi 文件问答接口**
- 🔍 提供提取证据，支持人工确认和修改
- 📦 覆盖 **7 大品类**：酒店、用车、线路、导游、餐饮、门票、活动
- 🏷️ 自动提取兴趣标签和服务标签
- ⚡ Celery 异步处理，支持批量导入

### 2. 双库资源管理 (`/app/domain/skus` + `/app/domain/cooperations`)
- 📋 **私有库**：机构专属资源，仅本机构可见
- 🌐 **公共库**：机构选择发布，供其他机构申请合作
- 🏷️ 统一标签体系：`tags_interest`（兴趣）+ `tags_service`（服务）
- 📦 7 大品类统一属性：`category` + `attributes`（品类专属字段）
- 💵 价格可见性：
  - 供应商：可见成本价 `base_cost_price`
  - 合作方：可见合作销售价 `base_sale_price`
  - 未合作：仅基础信息和销售价格
- 📊 批量调价、批量更新、批量删除
- 🔄 合作后可将公共库 SKU 复制到私有库
...
### 4. 合作管理 (`/app/domain/cooperations`)
- 📤 发起合作申请：向任意已发布公共库的机构申请
- ✅ 审核合作：处理他人发来的合作申请（通过/拒绝）
- 🔗 合作网络：双向且显式，可撤销，超时处理
- 💰 协商价格：针对特定合作方设置专属价格
- 🔔 变更通知：合作 SKU 有变更自动站内信通知

### 5. 报价管理 (`/app/domain/quotations`)
- 🛒 **只能从私有库选择 SKU**（含自己导入 + 已合作公共库）
- 📸 快照机制：报价生成时固化 SKU 价格与属性
- 📄 PDF 导出：文案和图片分离，标准行程 + 图片汇总
- 🔗 生成公开分享链接（无需登录访问）
- 💼 支持草稿、已发布状态管理

### 6. 通知中心 (`/app/domain/notifications`)
- 🔔 合作变更通知：申请通过/拒绝/终止
- 📝 SKU 更新提醒：合作方资源有变更
- 📊 未读数量统计
- ✅ 批量标记已读

### 7. 供应商管理 (`/app/domain/vendors`)
- 🏢 供应商 CRUD 操作
- 📝 供应商备注管理
- 🎨 AI 自动生成供应商 Logo

### 8. 认证与多租户 (`/app/domain/auth`)
- 🔐 JWT Token 认证
