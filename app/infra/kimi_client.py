...
**酒店/住宿特征（优先级最高）：**
- 有明确的酒店名称（如"XX大酒店"、"XX宾馆"、"XX饭店"、"XX酒店"、"XX民宿"）
- 有房型描述（标准间、豪华套房、商务单间、套房、家庭房、大床房、双床房等）
- 包含房间面积（如"80m2"、"50平米"）
- 标题包含"X天X晚"且同时提到房型/住宿（如"家庭房3天2晚"）
- 有按房型和季节划分的价格表（旺季/平季/淡季或其他季节定价）
- 可能包含餐饮、会议室价格（但主要内容是房间定价）
- 即使同时提供餐饮和会议服务，只要有房型价格表就是hotel
→ 这是 **hotel（酒店）**，不是activity或itinerary！

**重要：如果标题同时包含房型词汇（家庭房/标准间/套房/大床房等）和住宿天数（X天X晚），必须识别为hotel！**
...
7. **正确识别 SKU 类型** - 根据实际内容判断（优先级从高到低）：
   ...
   - 如果标题包含房型词汇（家庭房/标准间/套房/大床房/双床房）+ 住宿天数（X天X晚）→ hotel
   - 如果包含房间面积（XXm2/XX平米）+ 住宿相关词汇 → hotel
   - 如果包含"N天N晚"、"双飞"、"双卧"、"行程"等多日旅游套餐但无房型 → itinerary
   - 如果只有餐饮价格 → restaurant
   - 如果有车辆/用车服务 → car
   - 如果有导游服务 → guide
   - 如果有门票信息 → ticket
...
{{
  ...
  // - 文档包含"N天N晚"、"双飞"、"双卧"、"行程"、"游东北"等多日旅游套餐但无房型 → 必须是 "itinerary" (category: "route")
...
**HOTEL（酒店/住宿）：**
  ...
  - room_types: 房型数组，结构：
    {{
      ...
      "room_type_name": "豪华套房/豪华标间/商务单间/家庭房/大床房/双床房等",
      "area_sqm": 80,  // 房间面积（如有）
  ...
  **酒店识别关键特征（必须遵守）：**
  - 标题包含房型词汇（家庭房/标准间/套房/大床房/双床房）→ 必须是hotel
  - 包含房间面积（如"80m2"、"50平米"）→ 必须是hotel
  - 标题格式"XXX 房型名称 X天X晚"（如"小小探索家 80m2翠雅居家庭房3天2晚"）→ 必须是hotel
...
**ITINERARY（行程/旅游套餐）：**
  ...
  - price_tiers: 价格档位数组（如果有多个价格），结构：
    [
      {{"dates": "4月15/20日", "price_per_person": 13999, "currency": "CNY"}},
      {{"dates": "5月11/18/25日、6月8/15日", "price_per_person": 14699, "currency": "CNY"}}
    ]
  ...
  **行程重要提示：**
  ...
  - 提取所有出发日期和对应价格（注意：同一张海报可能有多个价格档位）
  - 重点提取图片中间蓝色区域的图标内容作为highlights（通常包含航空、住宿、赠送等）
  - 不要添加海报中未提及的景点或服务
  - 价格通常在海报底部，格式如"13999元/人"或"14699¥/人"
