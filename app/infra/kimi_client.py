...
🏨 **酒店价格表特征（最高优先级）：**
- 文档标题包含"宾馆"、"酒店"、"饭店"、"大酒店"、"Hotel"等关键词
- 有房型列表（标准间、豪华套房、商务单间、套房、豪华标间等）
- 有按房型和季节划分的价格表（旺季/平季/淡季或其他季节定价）
- 可能包含餐饮、会议室价格（但主要内容是房间定价）
- **关键判断：只要文档中有"房型+价格表"结构，就必须是hotel**
- 即使同时提供餐饮和会议服务，只要有房型价格表就是hotel
→ 这是 **hotel（酒店）**，sku_type必须设为"hotel"，category必须设为"hotel"
...
7. **正确识别 SKU 类型** - 根据实际内容判断（优先级从高到低，严格按顺序检查）：
   - 🏨 **第一优先级：如果文档标题含"宾馆/酒店/饭店"或有房型价格表 → 必须是hotel**（即使同时提供餐饮和会议服务）
   - ✈️ 如果包含"N天N晚"、"双飞"、"双卧"、"行程"等多日旅游套餐 → itinerary
   - 🍽️ 如果只有餐饮价格（无房型） → restaurant
   - 🚗 如果有车辆/用车服务 → car
   - 👨‍🏫 如果有导游服务 → guide
   - 🎫 如果有门票信息 → ticket
   - 🎯 如果只有单项活动/体验项目（无房型、无多日行程） → activity
   
   **⚠️ 关键：如果提取了room_types字段，sku_type和category都必须是hotel，绝不能是activity！**
